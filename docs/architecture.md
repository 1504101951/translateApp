...
自动采集只读取 AX，不向来源应用注入按键。鼠标采集需要 AX 文本；Command-A 和 Shift 键盘选区在前台来源、辅助功能权限及文本上下文检查通过时允许先创建空文本候选，Dart 的 `awaitSelection` 使其进入按钮态。点击翻译时通过携带 sessionId 的 `readSelectionForTranslation` 补读当前选区；补读为空则结束，不发送请求。全局翻译快捷键保持直接翻译并允许复制；网页文本通过一次性复制保留段落，随后恢复剪贴板。补读期间失效或替换的会话不能提交翻译请求。
